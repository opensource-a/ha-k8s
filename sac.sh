...
aws cloudformation create-stack --stack-name $aws_stackname-dashboard-tg --template-url https://$accountid-$aws_stackname-cft.s3.amazonaws.com/deploy-dashboard/dashboard-tg.yaml --capabilities CAPABILITY_NAMED_IAM --parameters ParameterKey=AlbVpcId,ParameterValue=$nlb_vpc ParameterKey=DashboardNodePort,ParameterValue=$NodePort 
aws cloudformation wait stack-create-complete --stack-name $aws_stackname-dashboard-tg
...
aws elbv2 modify-listener --listener-arn $dashboard_listener_arn --default-actions Type=forward,TargetGroupArn=$targetGroupArn
